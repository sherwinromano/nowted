<x-edit-form-layout>
    ...
    <div class="flex flex-col p-8 w-full gap-8">
        <h1 class="text-white font-bold text-2xl">{{ $note->title }}</h1>
        <form action="{{ route('notes.update', $note->id) }}" method="POST" class="flex flex-col h-full justify-between">
            ...
            <button type="submit" class=" bg-green-700 text-black font-bold cursor-pointer px-8 py-2 rounded-md w-fit mt-4">Update</button>
        </form>
    </div>
</x-edit-form-layout>